function reducer(
  ...
  switch (action.type) {
    ...
    case "AddPurchaseAndUpdateProduct":
      let prodIndex = state.products.findIndex(
        (el) => el._id === action.payload.prodId
      );
      let newProdArr = state.products;
      newProdArr[prodIndex] = action.payload.prodObj;

      return {
        ...state,
        purchases: [...state.purchases, action.payload.pObj],
        products: newProdArr,
        purchaseAmount: state.purchases.length,
      };

    case "UpdateProduct":
      let prodInd = state.products.findIndex(
        (el) => el._id === action.payload.prodId
      );
    ...
    case "deleteProdFromPurchaseAnProducts":
      ...
      let newPurchase = state.purchases.filter(
        (el) => el.productid !== action.payload.prodid
      );
    ...
    case "UpdateCustomer":
      let customerInd = state.customers.findIndex(
        (el) => el._id === action.payload.custId
      );
    ...
    case "deleteCustomerFromPurchaseAnCustomers":
      let newCustomers = state.customers.filter(
        (el) => el._id !== action.payload.customerid
      );
      let newPurchases = state.purchases.filter(
        (el) => el.customerid !== action.payload.customerid
      );
